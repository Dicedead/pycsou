...
from pyxu.operator import DiagonalOp, Gradient, IdentityOp, L1Norm, PositiveOrthant
...
num_n = 1000
bin_size = 1
slm_pixels_height = 128
slm_pixels_width = 256
lambda_1 = 40
lambda_2 = 30
diff_lip = 500_000
weighted_heavy = True
...
class BhattLossWeighted(pxa.DiffFunc):
    def __init__(
        ...
        self._xrt = xrt
        self._reg = lambda_1 * PositiveOrthant(xrt.codim_shape).moreau_envelope(mu2)
...
class ReconstructionTechnique:
    ...
    def __run_epoch(self, x0: pxt.NDArray, mu1: pxt.Real, mu2: pxt.Real, stop_crit: pxa.StoppingCriterion):
        ...
        tvnorm = lambda_2 * L1Norm(grad.codim_shape).moreau_envelope(mu2) * grad
        pgd = PGD(loss + tvnorm)
