...
class BhattLoss(pxa.DiffFunc):
    def __init__(self, xrt: xray.RayXRT, ground_truth: pxt.NDArray, mu: pxt.Real, dh: pxt.Real = dh, dl: pxt.Real = dl):
        ...
        fg_posort = PositiveOrthant(dim_shape=xrt.dim_shape).argshift(fg_argshift).moreau_envelope(mu)
